"""
Service for å hente værdata fra YR (met.no) API
"""
import requests
...
class YRWeatherService:
    ...
    def get_weather_data(self, latitude: float, longitude: float, target_time: datetime) -> Optional[Dict[str, Any]]:
        ...
        Args:
            ...
            target_time: Tidspunkt vi ønsker værdata for
            
        Returns:
            Dict med værdata eller None hvis feil
        """
        try:
            # YR API parametere
            params = {
...
class OceanForecastService:
    ...
    def _extract_wave_for_time(self, ocean_data: Dict, target_time: datetime) -> Dict[str, Any]:
        ...
        return {
            'wave_height': instant_data.get('sea_surface_wave_significant_height'),
...
def calculate_wind_offshore(wind_direction: float, spot_orientation: float) -> bool:
    ...
    Args:
        wind_direction: Vindretning i grader (hvor vinden kommer fra)
        spot_orientation: Spot-orientering i grader (hvilken retning spot-et vender)
        
    Returns:
        True hvis vind er offshore
    """
...
def calculate_swell_component(wave_height: float, wave_direction: float, spot_orientation: float) -> float:
    ...
    Returns:
        Effektiv bølgehøyde basert på innfallsvinkel
    """
